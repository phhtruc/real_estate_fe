        ...
        <div class="iq-card-body">
          <form @submit.prevent="updateProperty">
            ...
            <div class="text-right">
              <button class="btn btn-primary" @click="updateProperty" :disabled="isLoading">
...
const updateProperty = async () => {
  ...
  try {
    await axios.put(`${rootAPI}/properties/${route.params.id}`, formData, {
...
const fetchPropertyDetails = async () => {
  try {
    ...
    property.value = {
      ...data, // Giữ nguyên các trường khác
      propertyType: data.propertyType.name || null, // Đảm bảo propertyType là object
    }

    filePondFiles.value = property.value.images.map((url) => ({
      source: url,
      options: { type: 'remote' }, // Đánh dấu là ảnh có sẵn
    }))
  } catch (error) {
